...
keydownR:
	...
	if(keyState[MojiCode] != 0 && (g_KeyRepeat = 1 || kName = "Space"))
	{
	 	; キーリピートの処理
		g_OyaRKeyOn := 1
		g_OyaRTick := Pf_Count()				; A_TickCount
	...
	if(keyState[MojiCode] == 0) 	; キーリピートの抑止
	{
		g_OyaRState := 1
		g_OyaRKeyOn := 1
		g_OyaRTick := Pf_Count()				;A_TickCount
...
Interrupt10:
	...
	if(keyState["A04"] != 0)
	{
		...
		{
			; ひらがな／カタカナキーはキーアップを受信できないから、0.2秒でキーアップと見做す
...
gSC002:	;1
	kName := "1"
	MojiCode := "E01"
	g_metaKey := keyAttribute2[g_Romaji . MojiCode]
